...
document.addEvent("click",f.document);}}},this);},attachResize:function(d,c){var b=window.retrieve("rokpad:events:resize",function(){var g=document.getElement(".btn-toolbar"),e=this._calculateHeight(c),f=g.getSize().y+18;
...
j=document.getElement(".btn-toolbar.rokpad-fullscreen");f.setStyles(q).inject(n.element,n.position);j.removeClass("rokpad-fullscreen").inject(c);f.getElement(".rokpad-editor-container").setStyles(k);
if(m.html["overflow-y"]=="scroll"||m.html.overflow=="auto"){document.id(document.html).setStyle("overflow",m.html.overflow);}document.id(document.body).setStyle("overflow",m.body);
window.scrollTo(e.x,e.y);this.editors[d].getEditor().resize.delay(1,this.editors[d].getEditor());this.detachResize();f.removeClass("rokpad-cantresize");
o=f.getElement("[data-rokpad-container]").retrieve("resizer");o.attach();g.getElement("i").className="rokpad-icon-fullscreen";f.store("rokpad:states:fullscreen",false);
}else{j=f.getNext("#editor-xtd-buttons .btn-toolbar");f.store("rokpad:states:location",this._getLocation(f));f.store("rokpad:states:styles",f.getStyles("position","left","top","right","bottom","z-index","height"));
f.store("rokpad:states:containerheight",f.getElement(".rokpad-editor-container").getStyles("height","min-height"));f.store("rokpad:states:document",{html:document.id(document.html).getStyles("overflow","overflow-y","overflow-x"),body:document.id(document.body).getStyle("overflow")});
f.store("rokpad:states:documentscroll",window.getScroll());f.inject(document.id(document.body));j.inject(f,"after");var p=this._calculateHeight(f),h=j.getSize().y+13;
f.setStyles({position:"fixed",left:0,top:0,right:0,bottom:h,"z-index":10000,height:"auto"});j.addClass("rokpad-fullscreen");f.getElement(".rokpad-editor-container").setStyles({height:window.getSize().y-p-h,"min-height":"0"});
